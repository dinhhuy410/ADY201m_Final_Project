...
CSV_FILE_PATH = os.path.join("data", "trade_history.csv")
...
def fetch_and_engineer_data(ticker_code, start_dt, end_dt):
    ...
    # --- FEATURES BỔ SUNG CHO MODEL XGBOOST (khớp với lúc train) ---
    ...
    df['Hist_Volatility_5'] = df['Log_Return'].rolling(window=5).std()
    df['Hist_Volatility_20'] = df['Log_Return'].rolling(window=20).std()
...
def train_and_predict_models(df):
    xgb_model = joblib.load(os.path.join('models', 'vcb_xgb_model.pkl'))
    lstm_model = load_model(os.path.join('models', 'vcb_lstm_model.h5'), compile=False)
    tcn_model = load_model(os.path.join('models', 'vcb_tcn_model.h5'), compile=False)
    scaler = joblib.load(os.path.join('models', 'scaler.pkl'))

    # --- 1. Dự báo LSTM ---
    lstm_features = df[['Close', 'RSI', 'Volume']].tail(60).values
    scaled_lstm = scaler.transform(lstm_features)
    X_lstm = np.array([scaled_lstm])
    lstm_pred_scaled = lstm_model.predict(X_lstm, verbose=0)
    # Khôi phục giá trị thực
    dummy = np.zeros((1, 3))
    dummy[0, 0] = lstm_pred_scaled[0, 0]
    lstm_pred = float(scaler.inverse_transform(dummy)[0, 0])

    # --- 2. Dự báo XGBoost ---
    # Đảm bảo features của XGBoost khớp với lúc train (15 features trong file của bạn)
    features_xgb = ['Close', 'RSI', 'Volume', 'Return_1d', 'Return_3d', 'Return_5d', 
                    'SMA_5', 'SMA_20', 'Volume_MA_5', 'Volume_Change', 
                    'Hist_Volatility_5', 'Hist_Volatility_20', 'Close_lag_1', 'Close_lag_3', 'Volume_lag_1']
    X_xgb = df[features_xgb].iloc[-1].values.reshape(1, -1)
    xgb_pred_ret = xgb_model.predict(X_xgb)[0]
    xgb_pred = float(df['Close'].iloc[-1] * (1 + xgb_pred_ret / 100))

    # --- 3. Dự báo TCN ---
    # TCN trong file của bạn train trên 30 phiên
    tcn_features = df[['Close']].tail(30).values.reshape(-1, 1)
    # TCN cũng cần scaler của giá Close (tận dụng lại scaler nếu nó cùng train trên Close)
    tcn_scaled = scaler.transform(np.hstack([tcn_features, np.zeros((30, 2))]))[:, 0].reshape(1, 30, 1)
    tcn_pred_scaled = tcn_model.predict(tcn_scaled, verbose=0)
    dummy_tcn = np.zeros((1, 3))
    dummy_tcn[0, 0] = tcn_pred_scaled[0, 0]
    tcn_pred = float(scaler.inverse_transform(dummy_tcn)[0, 0])

    # --- 4. Ensemble ---
    final_ensemble = (xgb_pred * 0.4) + (lstm_pred * 0.3) + (tcn_pred * 0.3)
